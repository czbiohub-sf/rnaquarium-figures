...
# define model paths
zebrapretrained_model_to_use="250810_230429_geneformer_Zebrahub_100-3000_L6_emb256_SL2048_E30_B12_LR0.001_LSlinear_WU10000_Oadamw_DS1"
model_zebrahub="/path/to/Geneformer_RQfork/pretrain/Zebrahub/models/${zebrapretrained_model_to_use}/models" # model pretrained on zebrahub data


# define variables
# >>> check this path <<<
working_dir=/path/to/Geneformer_RQfork/finetune/zebrahub_model_zebrahub_100-3000_tune
...
for test_size in $(seq $lower_bound $step $upper_bound); do
    ...
    # echo "#########################"
    # echo "# training with XGBoost #"
    # echo "#########################"

    # module load cuda/12.5.1_555.42.06
    # export CUDA_HOME=/hpc/apps/x86_64/cuda/12.5.1_555.42.06
    # module load cudnn/9.0.0.3_cuda12
    # conda activate cuda12.5_xgb_cupy_cuml

    # xgb_script="/path/to/Geneformer/xgb/xgboost_train.py"
    # output_prefix="zebrahub_${finetune_task}_classifier_freeze${freeze_layers}layers_testsize${perc}"
    
    # python $xgb_script --working_dir . \
    #                     --output_prefix "${output_prefix}" \
    #                     --train_data_path "${output_prefix}/${fingerprint}/${output_prefix}_labeled_train.dataset" \
    #                     --eval_data_path  "${output_prefix}/${fingerprint}/${output_prefix}_labeled_test.dataset"\
    #                     --label_col_name "label" \
    #                     --n_procs $n_cpu \
    #                     --use_tmp \
    #                     --tmp_dir "/tmp/${SLURM_JOB_ID}/" \
    #                     --fingerprint "$fingerprint" \
    #                      2>&1 | tee "$working_dir/zebrahub_${finetune_task}_classifier_xgboost_testsize${perc}.log"

    # # load back to Geneformer2025 env
