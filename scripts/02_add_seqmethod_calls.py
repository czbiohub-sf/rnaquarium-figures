...
#
# python3 scripts/02_add_seqmethod_calls.py data/intermediate/stats_merged.csv data/rnaquarium_seq_type_calls_3.csv > data/intermediate/stats_merged_seqtype.csv
...
def main():
    ...
    parser.add_argument('method_calls', type=Path,
                        default=Path(__file__).parent.parent / "data" / "rnaquarium_seq_type_calls.csv")
    ...
    joined_df = pd.merge(types_df, stats_df, how='right', left_on='acc', right_on='id').drop(columns='acc')
